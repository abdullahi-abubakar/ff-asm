...
class TestSpecDocument:
    ...
    def test_spec_is_reachable_and_valid_json(self, openapi_spec):
        assert "paths" in openapi_spec, "OpenAPI document missing top-level 'paths'"
        assert "definitions" in openapi_spec, "OpenAPI document missing top-level 'definitions'"
    ...
    def test_spec_declares_integration_schema(self, openapi_spec):
        ...
        assert "id" in props, "model.Integration must declare id in properties"
        assert "name" in props, "model.Integration must declare name in properties"
        assert "tenant_id" in props, "model.Integration must declare tenant_id in properties"
        assert "type" in props, "model.Integration must declare type in properties"

    def test_spec_declares_asset_schema(self, openapi_spec):
        schema = openapi_spec["definitions"].get("model.Asset", {})
        props = schema.get("properties", {})
        assert "id" in props, "model.Asset must declare id in properties"
        assert "name" in props, "model.Asset must declare name in properties"
        assert "integration_id" in props, "model.Asset must declare integration_id in properties"
        assert "tenant_id" in props, "model.Asset must declare tenant_id in properties"
...
class TestIntegrationContracts:
    ...
    def test_update_response_matches_schema(self, integration_u1: dict, integrations1: IntegrationClient):
        ...
        assert resp.status_code == 200, (
            f"PUT /integrations should return 200, got {resp.status_code}: {resp.text[:500]!r}"
        )
    ...
    def test_error_response_matches_http_error_schema(self, integrations1: IntegrationClient):
        ...
        assert resp.status_code == 404, (
            f"GET missing integration should return 404, got {resp.status_code}: {resp.text[:500]!r}"
        )
...
class TestAssetContracts:
    ...
    def test_create_response_matches_schema(self, integration_u1: dict, assets1: AssetClient):
        ...
        assert resp.status_code in HTTP_POST_CREATE_OK, (
            f"POST /assets should return 200 or 201, got {resp.status_code}: {resp.text[:500]!r}"
        )
    ...
    def test_update_reflects_new_name(self, asset_u1: dict, assets1: AssetClient):
        ...
        assert resp.json()["name"] == "shiny-new-name", (
            f"PATCH /assets should return updated name 'shiny-new-name', got {resp.json()['name']!r}"
        )
